...
`POST /cases/generate` runs the real AI pipeline (generator + logic judge + safety judge, reused directly from `tools/ai-content`) live, on a player's click, and streams real progress via SSE. Bounded by two independent, atomic, Firestore-backed daily counters (a 50/day success cap and a 2000/day attempt cap that also counts judge-rejected attempts — cost isn't bounded by successes alone) plus a 3/minute per-IP limit, and up to 10 retries per request. Real cost, measured against the deployed production service (not just estimated): a single-attempt success cost `$0.00053`; a fully-exhausted 5-attempt request cost `$0.00224` total. Worst case at the attempt cap stays under ~$1.20/day — well inside the existing €10 budget alert, with wide margin. Every result is stored with `source="live_generated"`, distinguishing it from curated content; the actual daily case is never touched by this feature. A real `429` (temporarily lowered cap) and real judge rejections with the visible SSE restart were both directly observed against the live deployment. **The real production pass rate turned out to be much lower than pre-deployment estimates (~5-10% per attempt, not ~29%)** — investigated by inspecting real rejected candidates directly, confirmed as a genuine generator quality issue (not judge over-strictness), addressed by raising the retry/attempt budgets since real per-attempt cost is trivial. Full reasoning, including the full investigation and what did and didn't work: [ADR-0007](architecture-decisions/ADR-0007-live-case-generation.md).
