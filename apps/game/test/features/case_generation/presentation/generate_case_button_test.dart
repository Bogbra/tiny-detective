...
void main() {
  ...
  testWidgets('shows the live checklist and a visible restart after a rejection', (
    tester,
  ) async {
    ...
    controller.add(const GenerationEvent(step: 'generating', status: 'running', attempt: 1));
    await tester.pump();
    expect(find.byType(CircularProgressIndicator), findsOneWidget);

    controller.add(const GenerationEvent(step: 'generating', status: 'passed', attempt: 1));
    controller.add(const GenerationEvent(step: 'logic_check', status: 'running', attempt: 1));
    controller.add(
    ...
    controller.add(const GenerationEvent(step: 'generating', status: 'running', attempt: 2));
    await tester.pump();
  ...
  testWidgets('quota exhausted shows a clear message with a dismiss action, not a dead end', (
    tester,
  ) async {
    final controller = StreamController<GenerationEvent>();
    final repository = FakeCaseGenerationRepository(events: controller.stream);
    ...
    controller.add(
      const GenerationEvent(step: 'quota_check', status: 'rejected', detail: 'daily generation quota reached'),
    );
    await tester.pump();

    expect(find.textContaining('generation limit is reached'), findsOneWidget);
  ...
  testWidgets('pipeline exhaustion offers a retry, not a hang', (tester) async {
    ...
    expect(find.textContaining('could not generate a valid case'), findsOneWidget);
    expect(find.text('Retry'), findsOneWidget);

    await controller.close();
  });

  testWidgets('a successful generation is reflected in generatedCase', (tester) async {
    final controller = StreamController<GenerationEvent>();
    final repository = FakeCaseGenerationRepository(events: controller.stream);
    final viewModel = CaseGenerationViewModel(repository);
    ...
    controller.add(GenerationEvent(step: 'saving', status: 'done', generatedCase: _sample()));
    await tester.pump();
