...
- **Live case generation (`POST /cases/generate`) is a scoped, documented exception to "no unvetted AI case reaches players"** — opt-in, still gated by both real AI judges plus a domain consistency check, capped by a global daily budget (a success cap the player sees, and a separate, higher attempt cap that also counts judge-rejected attempts, since those cost tokens too and aren't otherwise bounded), and every result is tagged `source="live_generated"` so it stays distinguishable from curated content. Real cost measured against the live deployment: $0.00053 for a successful attempt, $0.00224 for a fully-exhausted 5-attempt request. See [ADR-0007](docs/architecture-decisions/ADR-0007-live-case-generation.md).
- **The initial ~29% retry-count estimate for live case generation was small-sample noise — the real production rate is closer to 5-10%.** Found live (players hitting "could not generate a valid case" often enough to report it), root-caused by reading real rejected candidates directly (a genuine generator quality issue — solutions "matching" a clue property the clue itself never actually stated — not the judge being overly strict), and investigated with 40 further real API calls isolating prompt version and temperature as variables. Both prompt fixes shipped (`generate_case_v2.md`, `evaluate_case_logic_v2.md`) are real improvements but did not measurably move the pass rate on gpt-4o-mini — reported as a negative result, not glossed over. Retry/attempt budgets were raised instead (10 attempts, 2000/day cap), since real per-attempt cost is trivial regardless. See [ADR-0007](docs/architecture-decisions/ADR-0007-live-case-generation.md)'s addendum.
